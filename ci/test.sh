...
load_test() {
	...
	if [[ -f "${test_file_base}.expected_output" ]]; then
		if [[ $(diff "${test_file_base}.output" "${test_file_base}.expected_output" | wc -c) -gt 0 ]]; then
			echo "Got output ${test_file_base}.output:"
			cat "${test_file_base}.output"
			ERRORS+=("${test_name}: Invalid output")
		fi
	fi
